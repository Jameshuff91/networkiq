...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === 'install') {
    // Comment out welcome page until deployed
    // chrome.tabs.create({
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  if (changeInfo.status === 'complete' && tab.url?.includes('linkedin.com')) {
    ...
    // trackEvent('linkedin_tab_opened', { url: tab.url });  // Disabled until analytics
    console.log('LinkedIn tab opened:', tab.url);
  }
});

// Note: Don't add chrome.action.onClicked when using default_popup in manifest
