...
class NetworkIQUI {
  ...
  async injectSearchScores() {
    ...
    for (let i = 0; i < allProfiles.length; i++) {
      ...
      // Count by tier
      if (scoreResult.tier === 'gold') {highScoreCount++;} else if (scoreResult.tier === 'silver') {mediumScoreCount++;} else {lowScoreCount++;}
...
${this.scorer.generateMessage(profile, scoreData)}
  ...
  calculateQuickScore(profile) {
    ...
    if (text.includes('usafa') || text.includes('air force academy')) {score += 40;} else if (text.includes('military') || text.includes('veteran')) {score += 30;}
