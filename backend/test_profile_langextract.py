...
user_search_elements = [
    ...
    {"category": "education", "value": "mit", "weight": 30, "display": "MIT Alumni"},
    {
        ...
        "value": "google",
        "weight": 30,
        "display": "Former Google",
    },
    {
        "category": "military",
        "value": "air force academy",
        "weight": 45,
        "display": "USAFA Graduate",
    },
    {
        "category": "military",
        "value": "air force",
        "weight": 35,
        "display": "Air Force Veteran",
    },
    {
        "category": "skill",
        "value": "machine learning",
        "weight": 15,
        "display": "ML Expertise",
    },
    {
        "category": "skill",
    ...
    for profile in test_profiles:
        ...
        if result["matches"]:
            ...
            for match in result["matches"]:
                print(
                    f"  ✓ {match['matches_element']} - {match['found_in_profile'][:50]}..."
                )
                print(
                    f"    Confidence: {match.get('confidence', 'N/A')}, Points: {match['points']}"
                )
...
def test_batch_analysis():
    ...
    results = analyzer.analyze_profiles_batch(
        test_profiles, user_search_elements, max_workers=3
    )
    ...
    for result in results:
        ...
        print(
            f"{name:20} | Score: {score:3} | Tier: {tier:6} | Matches: {matches} | Method: {method}"
        )
...
def test_comparison():
    ...
    # LangExtract results
    avg_confidence_with = sum(
        m.get("confidence", 0) for m in result_with.get("matches", [])
    ) / max(len(result_with.get("matches", [])), 1)
    print(
        f"{'LangExtract':<20} | {result_with['score']:<10} | {len(result_with.get('matches', [])):<10} | {avg_confidence_with:.2f}"
    )

    # Standard results
    avg_confidence_without = sum(
        m.get("confidence", 0) for m in result_without.get("matches", [])
    ) / max(len(result_without.get("matches", [])), 1)
    print(
        f"{'Standard Gemini':<20} | {result_without['score']:<10} | {len(result_without.get('matches', [])):<10} | {avg_confidence_without:.2f}"
    )
