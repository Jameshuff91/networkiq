...
class TestPipeline:
    ...
    def parse_resume(self, resume_path: str) -> Dict:
        ...
        for elem in search_elements[:5]:
            print(
                f"  - {elem['category']}: {elem['display']} (weight: {elem['weight']})"
            )
    ...
    def test_single_profile(self, profile: Dict, search_elements: List[Dict]) -> Dict:
        ...
        try:
            ...
            analysis = self.profile_analyzer.analyze_profile(
                profile_data=profile["data"], user_search_elements=search_elements
            )
            ...
            if analysis["matches"]:
                ...
                for match in analysis["matches"]:
                    ...
                    if confidence >= 0.3:  # Only show valid matches
                        ...
                        print(
                            f"      Confidence: {confidence:.1f}, Points: {match['points']}"
                        )
            ...
            result = {
                ...
                "tier": analysis["tier"],
                "matches": len(
                    [m for m in analysis["matches"] if m.get("confidence", 0) >= 0.3]
                ),
                "time": analysis_time,
            ...
            if match.get("confidence", 0) >= 0.3:
                ...
                if found_text in ["NULL", "NONE", "N/A", ""]:
                    ...
                    validation["issues"].append(
                        f"NULL value counted: {match['matches_element']}"
                    )

                # Check for wrong university matches
                if match["category"] == "education":
                    if "Johns Hopkins" in match[
                        "matches_element"
                    ] and "Johns Hopkins" not in profile["data"].get("education", ""):
                        validation["no_false_positives"] = False
                        validation["issues"].append(
                            f"Wrong university match: {match['matches_element']}"
                        )
    ...
    def test_batch_profiles(
        self, profiles: List[Dict], search_elements: List[Dict]
    ) -> None:
    ...
    def generate_report(self) -> None:
        ...
        print("\n📊 PERFORMANCE METRICS:")
        print(
            f"  Resume Parse Time: {self.performance_metrics.get('resume_parse_time', 0):.2f}s"
        )
        print(
            f"  Avg Profile Score Time: {self.performance_metrics.get('avg_profile_time', 0):.2f}s"
        )
        print(
            f"  Total Batch Time: {self.performance_metrics.get('batch_time', 0):.2f}s"
        )
        ...
        valid_results = [
            ...
            if r.get("validation", {}).get("no_false_positives", False)
        ]
        ...
        for result in self.results:
            print(f"\n  {result['profile']}")
            print(
                f"    Score: {result.get('score', 'N/A')} ({result.get('tier', 'N/A')})"
            )
            ...
            json.dump(
                {
                    ...
                    "summary": {
                        ...
                        "validation_pass_rate": (
                            len(valid_results) / len(self.results)
                            if self.results
                            else 0
                        ),
                    },
