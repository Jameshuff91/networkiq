...
JIM_RESUME_DATA = {
    "search_elements": [
        ...
        },
        {
            "category": "education",
            "value": "usafa",
            "display": "United States Air Force Academy",
            "weight": 45,
        },
        {
            "category": "education",
...
REAL_LINKEDIN_PROFILES = [
    {
        ...
        "expected": {
            ...
            "should_not_match": [
                ...
                "U.S. Air Force",
                "Anthropic",
            ],  # Anthropic is not in Jim's companies
    ...
    },
    {
        ...
        "expected": {
            "should_match": [
                "Machine Learning"
            ],  # Only skills should match, not company
            "should_not_match": [
                "United States Air Force Academy",
                "Scale AI",
            ],  # Scale AI is not in Jim's companies
...
class RealProfileTester:
    ...
    def test_profile(self, profile_data: Dict, search_elements: List[Dict]) -> Dict:
        ...
        try:
            ...
            for match in valid_matches:
                ...
                print(
                    f"    Confidence: {match['confidence']:.1f}, Points: {match['points']}"
                )
            ...
            print(f"\n❌ ERROR: {str(e)}")
            return {"profile": profile_data["name"], "error": str(e), "passed": False}
        ...
        for expected_match in expected.get("should_match", []):
            if not any(expected_match in m for m in matched_elements):
                ...
                self.validation_summary["missed_matches"].append(
                    {"profile": profile_data["name"], "missed": expected_match}
                )
        ...
        for should_not_match in expected.get("should_not_match", []):
            if any(should_not_match in m for m in matched_elements):
                ...
                self.validation_summary["false_positives"].append(
                    {"profile": profile_data["name"], "false_match": should_not_match}
                )
    ...
    def generate_report(self) -> None:
        ...
        print(
            f"  Success Rate: {(self.validation_summary['passed']/self.validation_summary['total_tests']*100):.1f}%"
        )
        ...
        print(
            f"  Max Response Time: {max(r.get('time', 0) for r in self.results):.2f}s"
        )
        print(
            f"  Min Response Time: {min(r.get('time', 0) for r in self.results if r.get('time', 0) > 0):.2f}s"
        )
